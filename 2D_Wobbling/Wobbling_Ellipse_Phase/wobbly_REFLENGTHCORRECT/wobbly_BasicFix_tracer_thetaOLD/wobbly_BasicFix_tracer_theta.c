...
{
  thetaNOW = 0.122173*sin((3.14159265/80.0)*t); // Derivation in notebook. Should write this up at some point.
  face vector av = a;
  foreach_face(y) {
...
event init(t = 0) {
  ...
  refine(level<MAXLEVEL && 0.01*sq(x) + sq(y) < sq(semiminor));
  unrefine(level<MAXLEVEL && 0.01*sq(x) + sq(y) > sq(semiminor));
...
event circle_flow (i++) {
  fraction (circle, (0.01*sq(x) + sq(y) - sq(semiminor)));
  foreach() {
    foreach_dimension() {
      u.x[] = (1. - circle[])*u.x[];
      u.y[] = (1. - circle[])*u.y[];
