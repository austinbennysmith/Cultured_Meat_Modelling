...
double bOil = 1.9e-5; // Diffusion coefficient of O2 in air
...
#define muOil 1.81e-4 // approximateley the viscosity of oil
...
#define PeWater ((refLength*refVelocity)/bWater) // Peclet number for water
#define PeOil ((refLength*refVelocity)/bOil) // Peclet number for oil
...
double Diffusion_StabilityWater; // test for stability of the FTCS diffusion scheme in Water
double Diffusion_StabilityOil; // stability test for the FTCS diffusion scheme in oil
...
int main() {
  ...
  fprintf(fp_params, "Water Diffusion coefficient: %g \n", bWater);
  fprintf(fp_params, "Oil Diffusion coefficient: %g \n", bOil);
  fprintf(fp_params, "Water Peclet Number: %g \n", PeWater);
  fprintf(fp_params, "Oil Peclet Number: %g \n", PeOil);
  fclose(fp_params);
...
event integration (i++) {
  ...
  foreach() {
    // advection-diffusion:
      qh[] = (f[]*(1.0/PeWater) + (1-f[])*(1/PeOil))*(TT[-1,0]-TT[0,0])/Delta + u.x[]*(TT[0,0]+TT[-1,0])/2.0 - ((sq(u.x[])*dt)/(2.0*Delta))*(TT[0,0]-TT[-1,0]);
      qv[] = (f[]*(1.0/PeWater) + (1-f[])*(1/PeOil))*(TT[0,-1]-TT[0,0])/Delta + u.y[]*(TT[0,0]+TT[0,-1])/2.0 - ((sq(u.y[])*dt)/(2.0*Delta))*(TT[0,0]-TT[-1,0]);
  ...
  // dt = dtnext (dt);
    ...
    // diffusion scheme:
    // qh[] = (1.0/Pe)*(TT[-1,0]-TT[0,0])/Delta;
    // qv[] = (1.0/Pe)*(TT[0,-1]-TT[0,0])/Delta;
...
event logstats (t += 1.0) {
    ...
    if (t<1.0) { // Could change this condition in the future but won't for now
      ...
      Diffusion_StabilityWater = (2*(1/PeWater)*DT)/sq(DeltaX); // DeltaX = DeltaY so it doesn't matter which one I put here
      Diffusion_StabilityOil = (2*(1/PeOil)*DT)/sq(DeltaX);
      ...
      fprintf(fp_params, "Water Diffusion Stability condition (must be <=0.5, see code for source): %g \n", Diffusion_StabilityWater);
      fprintf(fp_params, "Oil Diffusion Stability condition (must be <=0.5, see code for source): %g \n", Diffusion_StabilityOil);
...
double ybelow;
double yabove;
event stations (t = 0; t+=1.0; t<=tmax) // RC restricted the output a little, don't overdo it at first!
{
