...
#define Pe ((refLength*refVelocity)/b) // Peclet number
...
event integration (i++) {
  ...
  foreach() {
    // advection-diffusion:
      qh[] = (1.0/Pe)*(TT[-1,0]-TT[0,0])/Delta + u.x[]*(TT[0,0]+TT[-1,0])/2.0 - ((sq(u.x[])*dt)/(2.0*Delta))*(TT[0,0]-TT[-1,0]);
      qv[] = (1.0/Pe)*(TT[0,-1]-TT[0,0])/Delta + u.y[]*(TT[0,0]+TT[0,-1])/2.0 - ((sq(u.y[])*dt)/(2.0*Delta))*(TT[0,0]-TT[-1,0]);;
...
event logstats (t += 1.0) {
    ...
    if (t<1.0) { // Could change this condition in the future but won't for now
      ...
      Diffusion_Stability = (2*(1/Pe)*DT)/sq(DeltaX); // DeltaX = DeltaY so it doesn't matter which one I put here
