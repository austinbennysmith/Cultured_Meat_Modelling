...
if CHOICE == 2:
	...
	m, b = np.polyfit(TIMElist, VARIANCElist, 1)
	myfit = m*TIMElist+b
	# Here I plot the linear increase in variance as well as a best-fit line that gives the slope. The slope should equal the diffusion coefficient.
	plt.plot(TIMElist, myfit, label="fit: y="+str(round(m, 2))+"x+"+str(round(b, 2)))
	plt.plot(TIMElist, VARIANCElist)
