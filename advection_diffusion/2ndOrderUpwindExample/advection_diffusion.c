...
event integration (i++) {
  ...
  //   // dT[] = (U/(2*Delta))*(T[1,0]-T[-1,0]) + ((sq(U)*dt)/(2*sq(Delta)))*(T[1,0]-2*T[0,0]+T[-1,0]);
    ...
    // 2nd-order "upwind" advection (not really upwind since I'm not defining it based on the sign of U but I could make it upwind if I want in the futhre if it provides decent results):
  	// See the Wikipedia article "Upwind Scheme" for more. It is very helpful & straightforward.
  	dT[] = -U*((3*T[0,0]-4*T[-1,0]+T[-2,0])/(2*Delta)); // u- in Wikipedia's notation
  	// dT[] = -U*((-T[2,0]+4*T[1,0]-3*T[0,0])/(2*Delta)); // u+ in Wikipedia's notation
  ...
  foreach()
    ...
    // OR ALL IN ONE WITH THE PREVIOUS foreach() COMMENTED OUT:
