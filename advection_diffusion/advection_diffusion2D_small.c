...
double b = 0.01; // Diffusion coefficient
...
event printdata (t += 100.0; t < tmax) {
  foreach()
...
event integration (i++) {
  ...
  // // 	//Diffusion alone:
  	qh[] = -b*(T[0,0]-T[-1,0])/Delta;
    qv[] = -b*(T[0,0]-T[0,-1])/Delta;
...
event profiles (t = 0; t+=100.0; t<=tmax)
{
...
event gfsview (t += 100.0, t<=tmax) { // RC
    char name_gfs[200];
