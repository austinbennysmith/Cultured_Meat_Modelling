...
event profiles (t = 0; t+=1.0)
{
  ...
  for (double y = 0.2; y <= 0.4; y += 0.01)
    fprintf (fp, "%g %g %g\n", t, y, interpolate (u.x, .27, y));
  fclose (fp);

  fp = fopen("xprofBELOW", "a");
  for (double y = 0.0; y <= 0.2; y += 0.01)
    fprintf (fp, "%g %g %g\n", t, y, interpolate (u.x, .27, y));
  fclose (fp);
...
event blayer (t+=1.0) {
	...
	for (double x=.251; x<= 1.0; x += 0.01) {
		for (double y=0.2; y<=0.4; y+=0.01) {
			my_float = 100.0*(y-0.2);
			my_int = (int)my_float;
...
event blayer2 (t+=1.0) {
	...
	for (double x=0.251; x<= 1.0; x += 0.01) {
		for (double y=0.0; y<=0.2; y+=0.01) {
			my_float2 = 100.0*(y);
